...
import { Droplet, Filter, Search, ChevronLeft, ChevronRight } from 'lucide-react'
      ...
      <main className="px-12 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Update Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {requests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-md text-xs font-semibold ${req.statusColor}`}>
                        {selectedStatus[req.id] || req.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <select
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-center gap-2">
            ...
            <button className="w-8 h-8 bg-amber-500 text-white rounded-lg font-semibold">1</button>
