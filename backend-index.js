...
async function getDB() {
  if (cachedClient) {
    return cachedClient.db("lifeflowdb");
  }
...
// Get all blood donation requests (Admin only)
...
// Create blood donation request (Donor only)
...
// Get all blood donation requests (Volunteer)
...
// Volunteer responds to a blood request
app.post("/volunteer/respond/:requestId", logger, verifyToken, verifyRole("volunteer"), async (req, res) => {
  try {
    ...
    // Update request to mark that volunteer has responded
    await db
      .collection("bloodRequests")
      .updateOne(
        { _id: new ObjectId(requestId) },
